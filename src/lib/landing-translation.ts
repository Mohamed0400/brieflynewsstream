...
const pageCopy = {
  en: {
    ...
    fullCatalogSubtitle: (count: number) =>
      `${count.toLocaleString("en")} stories · ranked by impact`,
    topEditionIndicator: (count: number) => `Showing today's top ${count} by impact`,
    loadFullBriefing: (count: number) => `Browse all ${count.toLocaleString("en")} stories`,
  ...
  ar: {
    ...
    fullCatalogSubtitle: (count: number) =>
      `${count.toLocaleString("en")} خبر · مرتّب حسب الأثر في الأسواق`,
    topEditionIndicator: (count: number) => `عرض أهم ${count} خبراً اليوم حسب الأثر`,
    loadFullBriefing: (count: number) => `تصفّح كل ${count.toLocaleString("en")} خبر`,
